# -*- coding: utf-8 -*-
"""
Monte Carlo + 风险函数 R = alpha*(1-pi) + beta*(1-q) + (1-alpha-beta)*norm_t
用于寻找 BMI 分组边界与各组最佳 NIPT 时点（孕周 w*）
...
# === 1) 引入来自“第一段”的模型接口 ===
try:
    from Calclating_Pi_Q import pi_hat, q_hat, df as df_models
except Exception:
    import builtins
...
def _interval_sum(cumsum_row: np.ndarray, l: int, r: int) -> np.ndarray:
    """对 cumsum 的单行取 [l, r] 闭区间和（向量化对 NW 行同时做）"""
    if l == 0:
        return cumsum_row[:, r]
    return cumsum_row[:, r] - cumsum_row[:, l-1]

def _group_mean_over_w(l: int, r: int):
    """返回每个 w 上该组的 mean(pi)、mean(q) 以及样本数 n"""
    n = (r - l + 1)
    if n <= 0:
        return None, None, 0
...
def group_best_week_by_range(bmi_l: float, bmi_r: float,
    ...
    """
    使用前缀和快速求该 BMI 区间的最优 w*。
    如 restrict_to_observed=True，则仅在该组实际观测到的孕周范围内搜索。
    """
...
def evaluate_grouping(boundaries: np.ndarray,
                      ...
                      beta: float) -> dict:
    """
    boundaries: 升序数组，形如 [b0, b1, ..., bK]，K = n_groups
    要求：每组样本数 >= MIN_GROUP_N，否则该方案视为无效。
    返回：总体加权风险、各组信息（样本数、BMI 范围、最优周、组内最小风险）
    """
    K = len(boundaries) - 1
    groups = []
...
# === 5) 蒙特卡洛搜索分组边界（保持接口），无效分组会被 evaluate_grouping 拒绝 ===
def sample_boundaries(n_groups: int,
                      bmi_min: float,
                      bmi_max: float,
                      min_gap: float) -> np.ndarray | None:
    """
    随机生成升序的 BMI 分组边界（含两端），相邻至少 min_gap。
    """
    if n_groups < 2:
...
if __name__ == "__main__":
    ...
    # === Visualization (English-only labels) ===
    try:
