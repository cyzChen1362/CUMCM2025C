...
def to_weeks(val):
    """将 '11w+6', '11W+6', '13w', '13', 13.0 等转为小数周"""
    if pd.isna(val):
        return np.nan
...
def ensure_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'BMI' not in df.columns:
        if {'height','身高'}.intersection(df.columns):
            if 'height' not in df.columns and '身高' in df.columns:
                df = df.rename(columns={'身高': 'height'})
        if {'weight','体重'}.intersection(df.columns):
            if 'weight' not in df.columns and '体重' in df.columns:
                df = df.rename(columns={'体重': 'weight'})
        if 'height' in df.columns:
            h = pd.to_numeric(df['height'], errors='coerce')
            df['height_m'] = np.where(h>10, h/100.0, h)
        if 'weight' in df.columns:
            df['weight_kg'] = pd.to_numeric(df['weight'], errors='coerce')
        if {'height_m','weight_kg'}.issubset(df.columns):
...
def q_hat(w: float, bmi: float=None, feat: Dict[str, Any]=None) -> float:
    """外部唯一可用的 q：二维融合 q_fused2d(w,BMI)。"""
