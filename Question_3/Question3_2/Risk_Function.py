...
# === import trained assets & global df ===
from Calclating_Pi_Q import (
    pipe_pi,              # sklearn pipeline for pi(w,x)
    X_cols,               # feature columns expected by pipeline
    ETA_2D,               # fusion weight
    _w_grid2d, _b_grid2d, _q_grid_kernel2d,  # kernel grid for q
    w_centers, b_centers, _q_grid_bin2d,     # binned grid for q
    df as df_models                           # global cleaned dataframe with 'w' and 'BMI'
)
...
def process_group_file_fast(file_path: str, label: str) -> pd.DataFrame:
    ...
    q_mat = q_fused2d_vec(Wm, Bm)

    pi_mat = _predict_pi_mesh_fast(weeks, bmi)
